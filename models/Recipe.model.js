...
Recipe.create(data, (error, Recipe) => {
  ...
  console.log(`The title of the recipe is : ${title}`, Recipe);
});
...
Recipe.insertMany(allData, (error, recipes) => {
  if (error) {
    console.log('An error happened:', error);
    return;
  }
  console.log(`the titles of all recipies are ${title}`, recipes);
});

module.exports = Recipe;

recipeSchema.findOneAndUpdate({title : 'Rigatoni alla Genovese',}, {duration : 100 } )
.then(Recipe => 
  console.log('successful update'), Recipe)
.catch(error => console.log('an error has occured'), error);

recipeSchema.deleteOne({title : 'carrot cake'})
